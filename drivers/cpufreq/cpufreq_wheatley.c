...
/* This has been moved to cpufreq.c.
				      ...
				      const char *buf, size_t count)
{
	...
	dbs_info->prev_cpu_idle = get_cpu_idle_time(j,
						    &dbs_info->prev_cpu_wall, DEFAULT_IO_IS_BUSY);
...
//	unsigned long long deepidle_time, deepidle_usage;
	...
	cur_idle_time = get_cpu_idle_time(j, &cur_wall_time, DEFAULT_IO_IS_BUSY);
				...
				unsigned int event)
{
	...
	for_each_cpu(j, policy->cpus) {
	    ...
	    j_dbs_info->prev_cpu_idle = get_cpu_idle_time(j,
							  &j_dbs_info->prev_cpu_wall, DEFAULT_IO_IS_BUSY);
	    if (dbs_tuners_ins.ignore_nice) {
		j_dbs_info->prev_cpu_nice =
