...
if [ ! -d ../Zip_KaminariSL ]; then 
	mkdir ../Zip_KaminariSL;
	cp -rf ../Custom_AnyKernel/* ../Zip_KaminariSL;
fi;

# Make the modules dir if it doesn't exist
if [ ! -d ../Zip_KaminariSL/modules ]; then mkdir ../Zip_KaminariSL/modules; fi;

# Make the release dir if it doesn't exist
if [ ! -d ../Out_KaminariSL ]; then mkdir ../Out_KaminariSL; fi;

# Remove previous modules
if [ -d ../Zip_KaminariSL/modules ]; then rm -rf ../Zip_KaminariSL/modules/*; fi;

# Make wi-fi module dir
if [ ! -d ../Zip_KaminariSL/modules/pronto ]; then mkdir ../Zip_KaminariSL/modules/pronto; fi;

# Modules
find ./ -type f -name '*.ko' -exec cp -f {} ../Zip_KaminariSL/modules/ \;
mv ../Zip_KaminariSL/modules/wlan.ko ../Zip_KaminariSL/modules/pronto/pronto_wlan.ko;

# Copy zImage-dtb
cp -f arch/arm/boot/zImage-dtb ../Zip_KaminariSL/;
ls -l ../Zip_KaminariSL/zImage-dtb;
cd ../Zip_KaminariSL;

# Set zip name
if [ $version ] && [ "$version" != "" ]; then
	zipname="KaminariSL_v"$version"_Falcon";
else
	zipname="KaminariSL_"$builddate"_Falcon";
fi;
